...
def jangpro_mission_start():
    print("## JANGPRO AGENT (v_final_candidate): MISSION START ##")
    try:
        # 1. Upbit 데이터 호출
        print("[1/3] Calling Upbit API...")
        upbit_url = f"[https://api.upbit.com/v1/ticker?markets=](https://api.upbit.com/v1/ticker?markets=){','.join(TARGET_COINS)}"
        ...
        print("[3/3] Calling Gemini API with the standard model name...")
        # API 키 방식에서 가장 안정적으로 동작하는 표준 모델 이름으로 수정
